...
export async function solveSurvey(code) {
  ...
  const result = { ok: false, message: '', screenshotPath: null, code: null, logs };
  ...
  const run = async () => {
    ...
    const screenshot = async () => {
      const path = `/tmp/mcd-result-${Date.now()}.png`;
    ...
    for (let i = 0; i < 40; i++) {
      ...
      // Fehlerseite (z.B. 5-Tage-Regel, bereits teilgenommen, ungültiger Code) → keine Fragen + 100% / Fehlertext
      const isErrorPage =
        (qs.length === 0 && String(prog ?? '').trim() === '100') ||
        /nicht entgegennehmen|unable to accept|bereits teilgenommen|already (participated|completed)|5-tage|5 days|ungültig|invalid|nicht gültig/i.test(text);
      if (qs.length === 0 && isErrorPage) {
        result.message = `Umfrage nicht möglich:\n${cleanupText(text.slice(0, 500))}`;
        result.screenshotPath = await screenshot();
        return;
      }
      if (qs.length === 0) {
      ...
      if (sig === lastSig) {
        ...
        noProgress++;
        if (noProgress >= 3) {
          result.message = `⛔ Stecke auf der Seite fest (Frage kann nicht beantwortet werden). Screenshot anbei.\n${cleanupText(sig.split('|')[0].slice(0, 300))}`;
          result.screenshotPath = await screenshot();
          return;
        }
        await sleep(2500);
    ...
    if (voucherCode) {
      ...
      result.message = `Umfrage beendet (${prog ?? '?'}). Konnte keinen Code extrahieren – Screenshot anbei. Text:\n${cleanupText(text.slice(0, 600))}`;
    }
  };
...
async function readQuestions(page) {
  return page.evaluate(() => {
    ...
    for (const sec of document.querySelectorAll('section.question')) {
      ...
      const emailInputs = [...sec.querySelectorAll('input[type="email"], input[type="text"]')].filter(i => visible(i) && /mail|e-mail|email/i.test((i.placeholder || '') + ' ' + (i.getAttribute('aria-label') || '') + ' ' + (i.id || '')));
      ...
      if (radios.length || checkboxes.length || textInputs.length || textareas.length || selects.length || ranges.length || emailInputs.length) {
        out.push({ qid, radios, checkboxes: checkboxes.length, emailInputs: emailInputs.length, textInputs: textInputs.length, textareas: textareas.length, selects: selects.length, ranges: ranges.length, hasIframe });
      }
    }
    return out;
  });
...
async function answerQuestion(page, q, log) {
  ...
  if (q.emailInputs) {
    ...
    await page.evaluate((qid, mail) => {
      ...
      for (const el of sec.querySelectorAll('input[type="email"], input[type="text"]')) {
        if (!/mail|e-mail|email/i.test((el.placeholder || '') + ' ' + (el.getAttribute('aria-label') || '') + ' ' + (el.id || ''))) continue;
