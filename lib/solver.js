  ...
  const run = async () => {
    ...
    for (let i = 0; i < 40; i++) {
      ...
      for (const q of qs) {
        ...
        await answerQuestion(page, q, log);
      }
      ...
      if (sig === lastSig) {
        // keine Änderung → 1x wiederholen, dann aufgeben
        await sleep(2500);
      }
      lastSig = sig;
...
async function readQuestions(page) {
  return page.evaluate(() => {
    ...
    for (const sec of document.querySelectorAll('section.question')) {
      ...
      if (radios.length || checkboxes.length || textInputs.length || textareas.length || selects.length || ranges.length) {
        out.push({ qid, radios, checkboxes: checkboxes.length, textInputs: textInputs.length, textareas: textareas.length, selects: selects.length, ranges: ranges.length, hasIframe });
      }
    }
    return out;
  });
...
async function answerQuestion(page, q, log) {
  const questionLabel = await page.evaluate((qid) => {
    ...
    return el ? el.innerText.trim().slice(0, 100) : qid;
  }, q.qid);
